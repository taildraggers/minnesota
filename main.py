"""Aggregates Minnesota-located listings from every taildraggers.com
...
ones whose location ends in ", MN".
...
repos' approach, with the state filter swapped to "MN".
"""
...
PAGE_TITLE = "Taildragger Ads in Minnesota"
STATE_ABBR = "MN"
...
_MINNESOTA_LOCATION_RE = re.compile(r",\s*MN\s*$", re.IGNORECASE)


def _is_minnesota(location: str) -> bool:
    return bool(_MINNESOTA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        minnesota_rows = [row for row in rows if _is_minnesota(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(minnesota_rows)} in Minnesota")
        all_listings.extend(minnesota_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Minnesota listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Minnesota listings: {len(listings)}")
