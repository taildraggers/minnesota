...
Daily aggregator of Minnesota-located taildragger classified listings,
...
[Michigan](https://github.com/taildraggers/michigan) repos' approach, with
the state filter swapped to "MN".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MN` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Minnesota"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/minnesota/`
...
```html
<iframe
  src="https://taildraggers.github.io/minnesota/"
  title="Taildragger Ads in Minnesota"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MINNESOTA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
